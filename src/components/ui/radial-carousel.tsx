...
export const RadialCarousel: React.FC<RadialCarouselProps> = ({
  ...
  useEffect(() => {
    ...
    // Auto-rotate speed (degrees per millisecond)
    const speed = 0.02;

    const rotate = (time: number) => {
      if (lastTime !== null) {
        const delta = time - lastTime;
        if (!isPanning && isExpanded) {
          // Note: expanded means showing radial view in this component
          ...
          >
            <motion.div
              ...
              style={{
                ...
                height: responsiveSizes.centerSize + 60, // Add height for title
              }}
              className="relative flex flex-col overflow-hidden rounded-[32px] p-0 shadow-2xl transition-colors duration-300 sm:rounded-[42px] bg-white/20 backdrop-blur-sm"
            >
              <motion.img
                layoutId={`img-${items[activeIndex].id}`}
                src={items[activeIndex].url}
                alt={items[activeIndex].title}
                className="w-full flex-1 rounded-[32px] object-cover sm:rounded-[42px]"
                draggable={false}
              />
              <div className="h-[60px] flex items-center justify-center">
                <span className="text-xl font-bold text-slate-900 px-6 text-center line-clamp-1">{items[activeIndex].title}</span>
              </div>
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="overflow-hidden rounded-[18px] p-0 shadow-2xl transition-colors duration-300 sm:rounded-[24px]"
      >
        <motion.img
          layoutId={`img-${item.id}`}
          src={item.url}
          alt={item.title}
          className="h-full w-full rounded-[18px] object-cover sm:rounded-[24px]"
          draggable={false}
        />
      </motion.div>
    </motion.div>

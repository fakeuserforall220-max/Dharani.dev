...
export const CloudShader = ({
  ...
  useEffect(() => {
    ...
    const resize = () => {
      // Extremely low DPR for absolute maximum lag-free performance
      const dpr = 0.25;
    ...
    let lastDraw = 0;
    const fpsInterval = 1000 / 12; // Extremely low 12 FPS for max performance

    const draw = (now: number) => {
      frame = requestAnimationFrame(draw);
      if (!running) return;
      
      const elapsedFrame = now - lastDraw;
      if (elapsedFrame < fpsInterval) return;
      lastDraw = now - (elapsedFrame % fpsInterval);

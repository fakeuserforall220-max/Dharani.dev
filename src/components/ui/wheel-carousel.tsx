...
export interface WheelCarouselProps {
  ...
  photoWidth?: number;
  photoAspect?: "3/4" | "1/1" | "4/3" | "3/2" | "16/9";
  contentWidth?: number;
  gap?: number;
  photoRadius?: number;
...
export function WheelCarousel({
  ...
  const [resolvedMode, setResolvedMode] = useState<WheelCarouselMode>(mode);
  
  useEffect(() => {
    // Next themes fallback or just force dark for portfolio
  ...
  return (
    <motion.div
      ...
      className={cn(
        "flex h-full min-h-[500px] w-full items-center justify-center overflow-hidden",
        className,
      )}
    ...
    >
      ...
      >
        <div
          className="flex h-full shrink-0 items-center justify-center relative z-20"
          style={{
            width: `${photoWidth}%`,
        ...
        >
          <div
            className={cn(
              "relative w-full max-h-[80%] overflow-hidden border border-white/10 shadow-2xl rounded-2xl",
              photoClassName,
            )}
            style={{
          ...
          >
            ...
            {selectedItem.link && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="absolute inset-0 z-10 flex items-center justify-center opacity-0 hover:opacity-100 bg-black/40 backdrop-blur-sm transition-opacity duration-300"
              >
                <span className="px-6 py-3 bg-white text-black font-bold rounded-full shadow-xl">
                  View Project
                </span>
              </a>
            )}
        ...
        <div
          className="relative h-[150%] min-w-0 flex-1 overflow-hidden"
          style={{ maskImage: mask, WebkitMaskImage: mask }}
        >
          {showMarker && (
            <span
              aria-hidden="true"
              className="absolute top-1/2 z-10 -translate-y-1/2 rounded-full shadow-[0_0_20px_rgba(30,107,255,0.6)]"
              style={{
          ...
          {carouselItems.map((item, index) => {
            ...
            return (
              <div
                ...
                className={cn(
                  "pointer-events-none absolute top-1/2 origin-left whitespace-nowrap text-[clamp(1.5rem,3vw,3rem)] font-bold tracking-tight",
                  itemClassName,
                )}
                style={{
                  left: `${apexInset}%`,
                  ...
                  textShadow: selected ? "0 0 20px rgba(255,255,255,0.3)" : "none",
                }}
              >
                {item.label}
              </div>
